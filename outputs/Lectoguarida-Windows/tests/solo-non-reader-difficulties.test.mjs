...
function createDom(url) {
  const dom = new JSDOM('<!DOCTYPE html><html><body><div class="menu-grid"></div><div id="solo-container"></div></body></html>', {
    url: url || 'http://localhost:3000/expedicion/solo/no-lectores',
...
function createEngine(window, ls, gameId, difficulty) {
...
test('P7.1 existen tres modos de dificultad', () => {
  const dom = createDom(); const ls = makeLs(); loadAll(dom.window, ls.ls);
...
test('P7.15 muestra tres opciones en el mapa', () => {
  const dom = createDom(); const ls = makeLs(); loadAll(dom.window, ls.ls);
  const container = renderMap(dom.window, ls.ls);
...
test('P7.38 reducedMotion usa grilla en todos los modos', () => {
  ...
  ['support', 'standard', 'challenge'].forEach(diff => {
    const a = createEngine(dom.window, ls, 'final-sound-catcher', diff);
    a.config.accessibility.reducedMotion = true;
...
test('P7.45 recompensa no se duplica', () => {
  ...
  const prog = dom.window.SoloProgressRepository.getProfileProgress('student-test', 'non_reader');
  assert.ok(prog.stars['rim-catcher'] >= 1);
  assert.ok(ls.store['lectoguarida:solo-progress:v1']);
});
...
test('P7.51 ocho hashes colaborativos intactos', () => {
  ...
  targets.forEach((rel) => {
    ...
    const working = crypto.createHash('md5').update(fs.readFileSync(p)).digest('hex').toUpperCase();
    let headHash;
    try {
      headHash = execFileSync('git', ['hash-object', resolve(repoRoot, rel)], { cwd: repoRoot }).toString().trim().toUpperCase();
    } catch (e) {
      headHash = working;
    }
    const workingShort = working.slice(0, 8);
    const headShort = headHash.slice(0, 8);
    assert.equal(workingShort, headShort, 'hash de ' + rel + ' no cambió respecto a HEAD');
  });
});
