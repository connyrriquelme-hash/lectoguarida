...
const state = {
  ...
  micState: { support: 'checking', permission: 'unknown', recorder: 'idle', errorMessage: '', elapsedSeconds: 0 },
...
function clearRecording() {
  ...
  state.recordingPlaying = false;
  state.micState.recorder = 'idle';
  state.micState.elapsedSeconds = 0;
}

function syncMicSupport() {
  const secure = window.isSecureContext;
  const support = Boolean(secure && navigator.mediaDevices?.getUserMedia);
  state.micState.support = support ? 'supported' : 'unsupported';
  state.micState.permission = support ? state.micState.permission : 'denied';
  return support;
}

async function requestMicAccess() {
  syncMicSupport();
  if (state.micState.support !== 'supported') {
    state.micState.errorMessage = 'Tu navegador no permite grabar aquí.';
    state.micState.permission = 'denied';
    state.micState.recorder = 'error';
    render();
    return false;
  }
  try {
    ...
    state.micState.permission = 'granted';
    state.micState.recorder = 'ready';
    state.micState.errorMessage = '';
    render();
    return true;
  } catch (error) {
    state.micState.permission = 'denied';
    state.micState.recorder = 'error';
    state.micState.errorMessage = error?.name === 'NotAllowedError'
      ? 'No pudimos activar el micrófono. Revisa los permisos del navegador.'
      : 'No pudimos activar el micrófono en este dispositivo.';
    render();
    return false;
  }
}
...
async function startWebRecording() {
  if (!navigator.mediaDevices?.getUserMedia || !window.MediaRecorder) {
    state.micState.support = 'unsupported';
    state.micState.recorder = 'error';
    state.micState.errorMessage = 'Tu navegador permite practicar la lectura, pero no grabarla aquí.';
    render();
    return false;
  }
  const stream = await navigator.mediaDevices.getUserMedia({ audio: { echoCancellation: true, noiseSuppression: true, autoGainControl: true } });
  ...
  recorder.onstop = () => {
    ...
    state.recordingPlaying = false;
    state.micState.recorder = 'finished';
    state.micState.elapsedSeconds = Math.max(1, Math.round((Date.now() - (state.startedAt || Date.now())) / 1000));
  ...
  recorder.start(250);
  state.micState.recorder = 'recording';
  state.micState.errorMessage = '';
  return true;
}
...
function stopWebRecording() {
  ...
  if (state.micState.recorder === 'recording') state.micState.recorder = 'finished';
}
...
function resetToHome() {
  ...
  state.micState = { support: 'checking', permission: 'unknown', recorder: 'idle', errorMessage: '', elapsedSeconds: 0 };
  setSession('');
  render();
}
...
function readingView() {
  ...
  const micRecorder = state.micState.recorder;
  const micPermission = state.micState.permission;
  const micSupport = state.micState.support;
  const micError = state.micState.errorMessage;
  const showActivate = micSupport !== 'supported' || micPermission !== 'granted';
  const showRecord = micPermission === 'granted' && micRecorder !== 'recording';
  const showStop = micRecorder === 'recording';
  const showFinished = micRecorder === 'finished';
  return `
    ...
    <div class="reading-shell">
      <section class="card">
        ...
        <section class="reading-mobile-mic" aria-label="Micrófono de lectura">
          <div class="reading-recording-badge"><span aria-hidden="true">${showStop ? '⏺️' : '🎙️'}</span><strong>${showStop ? 'Grabando...' : showFinished ? 'Grabación lista' : showActivate ? 'Activar micrófono' : 'Listo para grabar'}</strong><small>⏱ <span id="readingTimer">${state.view === 'reading' ? (state.elapsedSeconds ? `${String(Math.floor(state.elapsedSeconds / 60)).padStart(2, '0')}:${String(state.elapsedSeconds % 60).padStart(2, '0')}` : '00:00') : '00:00'}</span></small></div>
          <div class="mic-orb ${state.listening ? 'listening' : ''}">🎙️</div>
          <h3>${showStop ? 'Te estoy escuchando…' : micError ? 'Hay un problema con el micrófono' : 'Pulsa para grabar tu lectura'}</h3>
          <p class="status">${micError || speechHint}</p>
          <div class="reading-mic-actions">
            ${showActivate ? `<button class="btn btn-primary" data-action="mic-activate" ${micSupport === 'supported' ? '' : 'disabled'}>🎤 Activar micrófono</button>` : ''}
            ${showRecord ? `<button class="btn btn-secondary" data-action="toggle-mic" ${canSpeech ? '' : 'disabled'}>🎙️ Iniciar grabación</button>` : ''}
            ${showStop ? `<button class="btn btn-danger" data-action="toggle-mic" ${canSpeech ? '' : 'disabled'}>⏹ Detener y guardar</button>` : ''}
            <button class="btn btn-secondary" data-action="play-recording" ${state.hasRecording ? '' : 'disabled'}>🔊 Escuchar grabación</button>
            <button class="btn btn-ghost" data-action="mic-retry" ${micError ? '' : 'disabled'}>↻ Probar otra vez</button>
            <button class="btn btn-ghost" data-action="continue-without-mic">Continuar sin micrófono</button>
            ${showFinished ? `<button class="btn btn-primary" data-action="continue-with-mic">Continuar</button>` : ''}
          </div>
        </section>
...
async function handleAction(event) {
  ...
  try {
    ...
    if (action === 'mic-activate') { await requestMicAccess(); return; }
    if (action === 'mic-retry') { state.micState.errorMessage = ''; state.micState.recorder = 'idle'; state.micState.permission = 'unknown'; return render(); }
    if (action === 'continue-without-mic') { state.micState.errorMessage = ''; state.micState.recorder = state.hasRecording ? 'finished' : 'idle'; state.view = 'comprehension'; return render(); }
    if (action === 'continue-with-mic') { state.micState.errorMessage = ''; state.view = 'comprehension'; return render(); }
...
function startTimerDisplay() {
  ...
  const readingTimer = app.querySelector('#readingTimer');
  if (!timer) return;
  const update = () => {
    ...
    if (readingTimer) readingTimer.textContent = label;
...
async function toggleMic() {
  try {
    syncMicSupport();
    if (state.micState.permission !== 'granted') {
      const ok = await requestMicAccess();
      if (!ok) return;
    }
    if (state.micState.recorder === 'recording') {
      stopWebRecording();
      state.micState.recorder = 'finished';
      render();
      return;
    ...
    const started = await startWebRecording();
    if (!started) {
      state.micState.recorder = 'error';
      state.micState.errorMessage = 'Tu navegador permite practicar la lectura, pero no grabarla aquí.';
      render();
